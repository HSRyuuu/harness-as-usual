...
1. `define-requirements`: material ambiguity가 있으면 `question-cN.md` 파일을 만들고 사용자가 `[Answer]:` field에 직접 답하게 한다. 사용자가 chat으로 답하면 질문-답변 매핑 표를 보여주고 확인받은 뒤 파일에 전사한다. 답변 파일을 검증한 뒤 최초 요청과 모든 answered question files를 종합해 단일 `requirements.md`를 작성/리뷰한다.
...
`direct-execute`는 clear, trivial, low-risk, reversible 작업을 위한 경량 종단 경로다. `start-work` 경유 경로는 topic audit을 유지하고, 명시적 direct invocation은 topic/audit 없이 실행한다. 두 경로 모두 high-risk 작업은 허용하지 않는다.
...
| Coding-topic workflow rules | `as-usual-rules/core-workflow.md` | coding topic 활성화 시 읽는 canonical workflow다. |
...
- Coding-topic runtime workflow는 `as-usual-rules/core-workflow.md`, find-cause runtime workflow는 `as-usual-rules/find-cause-workflow.md` 하나씩 유지한다. 이 둘만 runtime workflow prompt다.
...
- 사용자는 question file에 chat으로 답할 수도 있다. 이 경우 agent는 질문-답변 매핑 표를 먼저 보여주고 명시적 확인을 받은 뒤에만 `[Answer]:` field에 전사한다. 답변의 canonical record는 여전히 question file이다.
...
- `as-usual-rules/core-workflow.md`(coding topic)와 `as-usual-rules/find-cause-workflow.md`(find-cause issue)가 두 runtime workflow prompt다.
