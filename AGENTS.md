...
- 리뷰 판정 어휘는 `passed | findings | blocked`, 검증 판정 어휘는 `PASS | FAIL | INCONCLUSIVE`, 구현 완료 보고는 `DONE | NEEDS_CONTEXT | BLOCKED`를 사용한다. task 리뷰 상세는 `execute/task-<N>-requirements-review.md` / `execute/task-<N>-quality-review.md`, cleanup 리뷰 상세는 `clean-up/review-result-<type>.md`에 YAML frontmatter `verdict`와 함께 기록한다.
