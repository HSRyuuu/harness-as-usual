...
Runtime workflow는 target project의 하나의 topic에만 적용한다. Canonical topic folder는 아래 형식이다.
...
6. `finalize`: review 결과와 code cleanup 결정이 기록된 뒤 topic status를 정리하고, 사용자에게 post-finalize git action을 선택할지 묻고 멈춘다.
...
SessionStart hook은 AsUsual capability, `as-usual-rules/core-workflow.md` 경로, `using-as-usual` entrypoint, active topic 후보를 짧게 알려준다. Full core workflow는 주입하지 않고, AsUsual이 활성화될 때 agent가 파일에서 직접 읽는다. 이 hook이 주입됐다는 사실만으로 모든 요청에 workflow를 강제하지 않는다.
...
| Hook context injection | `hooks/session-start`, `hooks/run-hook.cmd`, `hooks/hooks.json`, `hooks/hooks-codex.json` | capability summary, core workflow path, entrypoint skill, active topic 후보를 주입한다. |
| Artifact templates | `templates/question.md`, `templates/requirements.md`, `templates/plan.md`, `templates/topic.md`, `templates/code-review-report.md`, `templates/report.md` | `.as-usual/topic/yyyy-MM-dd-<topic>/` 아래 생성되는 파일 shape다. `topic.md`와 `audit.jsonl`은 `scripts/topic-log.py init`으로 생성된다. |
...
| SessionStart hook | shell + JSON | `hooks/session-start` | `.as-usual/topic/` 후보 감지와 lightweight bootstrap context injection |
...
- Public docs는 `https://github.com/HSRyuuu/as-usual.git`와 `AS_USUAL_REPO`를 사용한다. `/Users/...` 같은 private absolute path를 public install docs에 넣지 않는다.
...
- `.as-usual/state.md`, `.as-usual/audit.md` 같은 project-global artifact를 만든다.
- `.as-usual/` 아래 project-global 상태/감사 artifact를 만든다.
...
- `.codegraph/`, `.as-usual/`, installed plugin cache output, local probe output을 커밋한다.
