...
- Before inventing flags, run the helper with `--help` or use these known forms:
  ...
  - `python3 /Users/happyhsryu/dev/personal/as-usual/scripts/topic-log.py record-task-review --topic-dir <topicDir> --task <task> --review-type task --status passed --critical 0 --important 0 --minor 0 --summary <text> --actor __AS_USUAL_HOST__`
  - `python3 /Users/happyhsryu/dev/personal/as-usual/scripts/topic-log.py complete-task --topic-dir <topicDir> --task <task> --mode test-required --test-target <test> --red-evidence <red-command-and-result> --green-evidence <green-command-and-result> --verification <text> --result PASS --artifacts <commaFiles> --actor __AS_USUAL_HOST__`
...
- During execute, verify the subagent path by recording at least one `task.dispatched` event with mode `subagent-driven`, followed by a task-level review event.
- During execute, every implementation task must use `complete-task --mode test-required` with non-empty `--test-target` and `--green-evidence` (add `--red-evidence` for a bug fix); do not use `--mode no-test` for this sandbox scenario.
