...
1. **subagent-driven task의 순차 이중 리뷰 비용 재평가** — 재검토 결과, 현재는 **합치거나 줄이지 않고 유지**한다. `executing-plan/SKILL.md`는 subagent가 구현한 각 task마다 requirements review(요구사항/plan/task contract 충족 여부) 후 quality review(코드 품질/유지보수성/검증 품질)를 순차 실행하도록 하며, 이는 Superpowers-style execution loop를 의도적으로 따른 것이다. 두 reviewer prompt의 초점이 분리되어 있고, requirements review가 통과하거나 disposition 처리된 뒤에만 quality review를 시작하는 순서가 scope drift를 먼저 막는다. 단일 prompt 통합은 호출 수를 줄일 수는 있지만 route-back 판단과 품질 판단을 섞어 게이트를 흐릴 위험이 있다. **후속 조건:** 실제 subagent-driven E2E에서 두 리뷰가 반복적으로 같은 finding만 내고, quality review의 고유 Critical/Important 발견이 거의 없으며, 리뷰 비용이 task 실행을 의미 있게 지연한다는 증거가 쌓일 때만 축소를 재검토한다. 축소가 필요하면 "한 prompt에서 requirements/quality 섹션을 순서대로 출력하고 `record-task-review`는 두 타입으로 계속 기록"하는 방식이 최우선 후보이며, 저위험 task 샘플링/조건부 생략은 그 다음 후보로 둔다.
2. **`scripts/topic-log.py` 1,524줄 단일 파일 분할** — **완료.** public runtime command path는 `scripts/topic-log.py`로 유지하고 내부 구현을 `scripts/as_usual_topic_log/` 패키지로 분리했다. 분리 범위는 core modules(`constants`, `paths`, `audit`, `status`, `validation`, `topic_md`, `utils`), command handler 그룹(`command_lifecycle`, `command_execution`, `command_review_finalize`, `command_memory`, `command_misc`), command facade, CLI parser/main이다. 검증은 `py_compile`, 양쪽 sandbox E2E helper tests 55개씩, manifest JSON, hook/helper/runtime surface smoke를 통과했다. 관련 커밋: `3a81741`, `c8d26de`, `d4347d3`, `05f7a47`, `2dbcb40`.
