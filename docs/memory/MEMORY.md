...
- **봉인된 레코드는 사후에 고치지 않는다.** finalized·cancelled 유닛의 산출물 문서는
  ...
  시점에도 open이라는 보장이 없다.
- **`rg … && echo FAIL || echo PASS` 는 잘못된 이유로 통과한다.** 이 저장소의 검증
  스킬이 쓰는 이 관용구는 `rg` 가 *매치를 못 찾을 때*와 *파일·경로를 못 찾을 때*를
  구분하지 못한다. cwd가 어긋나 있거나 파일명이 틀리면 네 검사가 전부 `PASS` 로 나온다.
  또 `git grep` 계열 검사는 추적된 파일만 보므로 **새로 만든 미추적 파일은 스캔되지
  않고 통과한다**. 통과를 믿기 전에 대상 파일이 실제로 읽혔는지 한 줄로 확인한다.
