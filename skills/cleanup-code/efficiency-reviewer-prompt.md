...
Read the topic artifacts, changed code, and nearby performance-sensitive paths. Your review is read-only except for writing the Efficiency section of `review.md`.
...
Write detailed findings to the Efficiency section of `review.md` with YAML frontmatter: `type: efficiency`, `verdict: passed | findings | blocked`, and `reviewedAt`. If no safe efficiency cleanup exists, create the file with `verdict: passed` and state that no safe efficiency cleanup was found.

Return only this receipt:

Verdict: passed | findings | blocked
Review File: review.md (Efficiency section)
Findings:
```
