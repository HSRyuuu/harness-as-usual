...
description: Use to recall relevant AsUsual long-term memory from docs/memory/ for the current task. Read-only utility, not a workflow phase; typically dispatched as a subagent during context gathering.
---

# Search Long-Term Memory

Reads `docs/memory/` and returns only what is relevant to the task at hand. It
never writes. Prefer dispatching it as a subagent so the caller's context stays
clean.
